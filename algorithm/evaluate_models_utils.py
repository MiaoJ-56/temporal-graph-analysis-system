...
def evaluate_model_link_prediction(model_name: str, full_data: Data, train_data: Data, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer', 'DyGFormer0', 'DyGFormer1', 'DyGFormer2', 'DyGFormer3', 'DyGFormer4', 'DyGFormer5']:
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                      ...
                                                                      num_neighbors=num_neighbors,
                                                                      time_gap=time_gap)
            elif model_name in ['DyGFormer', 'EasyMLP', 'DyGFormer0', 'DyGFormer1', 'DyGFormer2']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                      dst_node_ids=batch_neg_dst_node_ids,
                                                                      node_interact_times=batch_node_interact_times)
            elif model_name in ['DyGFormer3', 'DyGFormer4', 'DyGFormer5']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      ...
                                                                      node_interact_times=batch_node_interact_times,
                                                                      full_data=full_data,
                                                                      latest_training_time=
                                                                      train_data.node_interact_times[-1])
