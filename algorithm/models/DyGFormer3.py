...
class DyGFormer3(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        super(DyGFormer3, self).__init__()
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, full_data: Data, latest_training_time):
        """
        ...
        # 得到ts前的time_sample_num条数据 不合理，全部取出
        # 超参数 time_sample_num = ??? src_max_seq_length, dst_max_seq_length
        # 取个 patch_size * n; 取个512吧
        # self.time_sample_num = 512 * self.patch_size
        # (batch_size,
        src_recent_ids_list, dst_recent_ids_list, recent_edges_list, recent_times_list = \
            self.neighbor_sampler.get_recent_data(node_interact_times=node_interact_times, full_data=full_data, latest_training_time = latest_training_time)
        logger = logging.getLogger()

        # pad操作
        src_padded_nodes_recent_ids, dst_padded_nodes_recent_ids, recent_padded_nodes_edge_ids, recent_padded_nodes_neighbor_times = \
            self.recent_pad_sequences(node_ids=src_node_ids,
                                      recent_src_ids_list = src_recent_ids_list,
                                      recent_dst_ids_list = dst_recent_ids_list,
                                      nodes_edge_ids_list=recent_edges_list,
                                      nodes_neighbor_times_list=recent_times_list,
        ...
        # get recent_features
        src_padded_nodes_recent_raw_features, dst_padded_nodes_recent_raw_features, recent_padded_nodes_edge_raw_features, recent_padded_nodes_neighbor_time_features = \
            self.get_recent_features(node_interact_times=node_interact_times,
                                     src_padded_nodes_recent_ids=src_padded_nodes_recent_ids,
                                     dst_padded_nodes_recent_ids = dst_padded_nodes_recent_ids,
                                     padded_nodes_edge_ids=recent_padded_nodes_edge_ids,
                                     padded_nodes_neighbor_times=recent_padded_nodes_neighbor_times,
        ...
        # get the patches for recent
        src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features, \
        recent_patches_nodes_edge_raw_features, recent_patches_nodes_neighbor_time_features = \
            self.get_recent_patches(src_padded_nodes_recent_node_raw_features=src_padded_nodes_recent_raw_features,
                                    dst_padded_nodes_recent_node_raw_features=dst_padded_nodes_recent_raw_features,
                                    padded_nodes_edge_raw_features=recent_padded_nodes_edge_raw_features,
                                    padded_nodes_neighbor_time_features=recent_padded_nodes_neighbor_time_features,
        ...
        # align for recent  (batch_size, recent_num_patches, channel_embedding_dim)
        src_patches_nodes_recent_node_raw_features = self.projection_layer['node'](src_patches_nodes_recent_node_raw_features)
        dst_patches_nodes_recent_node_raw_features = self.projection_layer['node'](dst_patches_nodes_recent_node_raw_features)
        recent_patches_nodes_edge_raw_features = self.projection_layer['edge'](recent_patches_nodes_edge_raw_features)
        recent_patches_nodes_neighbor_time_features = self.projection_layer['time'](recent_patches_nodes_neighbor_time_features)
        ...
        recent_num_patches = src_patches_nodes_recent_node_raw_features.shape[1]

        # Tensor, shape (batch_size, src_num_patches + 2 * recent_num_patches + dst_num_patches + 2 * recent_num_patches, channel_embedding_dim)
        patches_nodes_neighbor_node_raw_features = torch.cat([src_patches_nodes_neighbor_node_raw_features, src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features,
                                                              dst_patches_nodes_neighbor_node_raw_features, src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features], dim=1)
        patches_nodes_edge_raw_features = torch.cat([src_patches_nodes_edge_raw_features, recent_patches_nodes_edge_raw_features, recent_patches_nodes_edge_raw_features,
                                                     dst_patches_nodes_edge_raw_features, recent_patches_nodes_edge_raw_features, recent_patches_nodes_edge_raw_features], dim=1)
        patches_nodes_neighbor_time_features = torch.cat([src_patches_nodes_neighbor_time_features, recent_patches_nodes_neighbor_time_features, recent_patches_nodes_neighbor_time_features,
                                                          dst_patches_nodes_neighbor_time_features, recent_patches_nodes_neighbor_time_features, recent_patches_nodes_neighbor_time_features,], dim=1)
        patches_nodes_neighbor_co_occurrence_features = torch.cat([src_patches_nodes_neighbor_co_occurrence_features, src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features,
                                                                   dst_patches_nodes_neighbor_co_occurrence_features, src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features], dim=1)
        ...
        # Tensor, shape (batch_size, src_num_patches + 2 * recent_num_patches + dst_num_patches + 2 * recent_num_patches, num_channels, channel_embedding_dim)
        patches_data = torch.stack(patches_data, dim=2)
        # Tensor, shape (batch_size, src_num_patches + 2 * recent_num_patches + dst_num_patches + 2 * recent_num_patches, num_channels * channel_embedding_dim)
        patches_data = patches_data.reshape(batch_size, src_num_patches + dst_num_patches + 4 * recent_num_patches, self.num_channels * self.channel_embedding_dim)

        # Tensor, shape (batch_size, src_num_patches + dst_num_patches + recent_num_patches * 4, num_channels * channel_embedding_dim)
        ...
        src_patches_data = patches_data[:, : src_num_patches + 2*recent_num_patches, :]
        # dst_patches_data, Tensor, shape (batch_size, dst_num_patches, num_channels * channel_embedding_dim)
        dst_patches_data = patches_data[:, src_num_patches + 2*recent_num_patches: src_num_patches + dst_num_patches + 4*recent_num_patches, :]
    ...
    def recent_pad_sequences(self, node_ids: np.ndarray ,
                             recent_src_ids_list: list,
                             recent_dst_ids_list: list,
                             nodes_edge_ids_list: list,
                             nodes_neighbor_times_list: list,
                             patch_size: int = 1,
                             max_input_sequence_length: int = 256):
        """
        pad the recent sequences for nodes in node_ids
        ...
        :return:
        """
        assert max_input_sequence_length > 0, 'Maximal number of recent nodes for each node should be greater than 1!'
        max_seq_length = 0
        # first cut the sequence of nodes whose number of neighbors is more than max_input_sequence_length (we need to include the target node in the sequence)
        # logger = logging.getLogger()
        # logger.info(len(node_ids))
        # logger.info(len(recent_src_ids_list))
        for idx in range(len(node_ids)):
            assert len(recent_src_ids_list[idx]) == len(recent_dst_ids_list[idx]) == len(nodes_edge_ids_list[idx]) == len(nodes_neighbor_times_list[idx])
            if len(recent_src_ids_list[idx]) > max_input_sequence_length :
                # cut the sequence by taking the most recent max_input_sequence_length interactions
                recent_src_ids_list[idx] = recent_src_ids_list[idx][-max_input_sequence_length:]
                recent_dst_ids_list[idx] = recent_dst_ids_list[idx][-max_input_sequence_length:]
                nodes_edge_ids_list[idx] = nodes_edge_ids_list[idx][-max_input_sequence_length:]
                nodes_neighbor_times_list[idx] = nodes_neighbor_times_list[idx][-max_input_sequence_length:]
            if len(recent_src_ids_list[idx]) > max_seq_length:
                max_seq_length = len(recent_src_ids_list[idx])
        ...
        src_padded_nodes_recent_ids = np.zeros((len(node_ids), max_seq_length)).astype(np.longlong)
        dst_padded_nodes_recent_ids = np.zeros((len(node_ids), max_seq_length)).astype(np.longlong)
        recent_padded_nodes_edge_ids = np.zeros((len(node_ids), max_seq_length)).astype(np.longlong)
        recent_padded_nodes_neighbor_times = np.zeros((len(node_ids), max_seq_length)).astype(np.float32)
        for idx in range(len(node_ids)):
            if len(recent_src_ids_list[idx]) > 0:
                # logging.getLogger().info(f"len(recent_src_ids_list) = {len(recent_src_ids_list[idx])}")
                # logging.getLogger().info(f"src_padded_nodes_recent_ids.shape = {src_padded_nodes_recent_ids.shape}")
                src_padded_nodes_recent_ids[idx, 0: len(recent_src_ids_list[idx])] = recent_src_ids_list[idx][:]
                dst_padded_nodes_recent_ids[idx, 0: len(recent_dst_ids_list[idx])] = recent_dst_ids_list[idx][:]
                recent_padded_nodes_edge_ids[idx, 0: len(nodes_edge_ids_list[idx])] = nodes_edge_ids_list[idx][:]
                recent_padded_nodes_neighbor_times[idx, 0: len(nodes_neighbor_times_list[idx])] = nodes_neighbor_times_list[idx][:]

        # three ndarrays with shape (batch_size, max_seq_length)
        return src_padded_nodes_recent_ids, dst_padded_nodes_recent_ids, recent_padded_nodes_edge_ids, recent_padded_nodes_neighbor_times
    ...
    def get_recent_features(self, node_interact_times: np.ndarray,
                            src_padded_nodes_recent_ids: np.ndarray,
                            dst_padded_nodes_recent_ids: np.ndarray,
        ...
        """
        # Tensor, shape (batch_size, max_seq_length, node_feat_dim)
        src_padded_nodes_recent_node_raw_features = self.node_raw_features[torch.from_numpy(src_padded_nodes_recent_ids)]
        dst_padded_nodes_recent_node_raw_features = self.node_raw_features[torch.from_numpy(dst_padded_nodes_recent_ids)]
        # Tensor, shape (batch_size, max_seq_length, edge_feat_dim)
        recent_padded_nodes_edge_raw_features = self.edge_raw_features[torch.from_numpy(padded_nodes_edge_ids)]
        # Tensor, shape (batch_size, max_seq_length, time_feat_dim)
        recent_padded_nodes_neighbor_time_features = time_encoder(timestamps=torch.from_numpy(node_interact_times[:, np.newaxis] - padded_nodes_neighbor_times).float().to(self.device))

        # ndarray, set the time features to all zeros for the padded timestamp
        recent_padded_nodes_neighbor_time_features[torch.from_numpy(src_padded_nodes_recent_ids == 0)] = 0.0

        return src_padded_nodes_recent_node_raw_features, dst_padded_nodes_recent_node_raw_features, recent_padded_nodes_edge_raw_features, recent_padded_nodes_neighbor_time_features



    def get_recent_patches(self,
                           src_padded_nodes_recent_node_raw_features: torch.Tensor,
                           dst_padded_nodes_recent_node_raw_features: torch.Tensor,
        ...
        :return:
        """
        assert src_padded_nodes_recent_node_raw_features.shape[1] % patch_size == 0 and dst_padded_nodes_recent_node_raw_features.shape[1] % patch_size == 0
        num_patches = src_padded_nodes_recent_node_raw_features.shape[1] // patch_size
        ...
        for patch_id in range(num_patches):
            ...
            src_patches_nodes_recent_node_raw_features.append(src_padded_nodes_recent_node_raw_features[:, start_idx: end_idx, :])
            dst_patches_nodes_recent_node_raw_features.append(dst_padded_nodes_recent_node_raw_features[:, start_idx: end_idx, :])
            recent_patches_nodes_edge_raw_features.append(padded_nodes_edge_raw_features[:, start_idx: end_idx, :])
            recent_patches_nodes_neighbor_time_features.append(padded_nodes_neighbor_time_features[:, start_idx: end_idx, :])

        batch_size = len(src_padded_nodes_recent_node_raw_features)
        # Tensor, shape (batch_size, num_patches, patch_size * node_feat_dim)
        src_patches_nodes_recent_node_raw_features = torch.stack(src_patches_nodes_recent_node_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.node_feat_dim)
        dst_patches_nodes_recent_node_raw_features = torch.stack(dst_patches_nodes_recent_node_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.node_feat_dim)
        # Tensor, shape (batch_size, num_patches, patch_size * edge_feat_dim)
        recent_patches_nodes_edge_raw_features = torch.stack(recent_patches_nodes_edge_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.edge_feat_dim)
        # Tensor, shape (batch_size, num_patches, patch_size * time_feat_dim)
        recent_patches_nodes_neighbor_time_features = torch.stack(recent_patches_nodes_neighbor_time_features, dim=1).reshape(batch_size, num_patches, patch_size * self.time_feat_dim)
        return src_patches_nodes_recent_node_raw_features, dst_patches_nodes_recent_node_raw_features, \
               recent_patches_nodes_edge_raw_features, recent_patches_nodes_neighbor_time_features
