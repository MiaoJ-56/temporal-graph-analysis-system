...
class NeighborSampler:
    ...
    def get_recent_data(self, node_interact_times: np.ndarray, full_data: Data, latest_training_time):
        """
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        :param full_data: Data
        :return: src_recent_ids_list, dst_recent_ids_list, recent_edges_list, recent_times_list
        """
        #
        src_recent_ids_list, dst_recent_ids_list, recent_edges_list, recent_times_list = [], [], [], []
        for idx in range(len(node_interact_times)):
            now = min(latest_training_time, node_interact_times[idx])
            index = np.where(full_data.node_interact_times == now)[-1][-1]
            # logging.getLogger().info(f"{now}")
            if index != 0:
                src_recent_ids_list.append(full_data.src_node_ids[0: index])
                dst_recent_ids_list.append(full_data.dst_node_ids[0: index])
                recent_edges_list.append(full_data.edge_ids[0: index])
                recent_times_list.append(full_data.node_interact_times[0: index])
            else:
                src_recent_ids_list.append([])
                dst_recent_ids_list.append([])
                recent_edges_list.append([])
                recent_times_list.append([])
        return src_recent_ids_list, dst_recent_ids_list, recent_edges_list, recent_times_list
